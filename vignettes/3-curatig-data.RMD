...
df <- df %>% add_row(
        ...
        "AGE_MEAN" = mean(as.numeric(p$AGE)),
        "AGE_MEDIAN" = median(as.numeric(p$AGE)),
        "AGE_RANGE" = paste(as.character(range(as.numeric(p$AGE))),collapse= "-"),
        "SEX_F" = table(p$SEX)[1],
        "SEX_M" = table(p$SEX)[2],
        "SMOKING_STATUS" = NA,
...
##############################################################
saveRDS(geo,OUTPUT(c(TODAY,"_GSE_LungTissue-CURATED.RDS")))

write_csv(df,OUTPUT(c(TODAY,"_GSE_Summary.csv")))
