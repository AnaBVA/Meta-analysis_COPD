...
for (i in 1:length(gs)) {
  gene <-  na.omit(full_tables[full_tables$Gene.Symbol == gs[i],])
  yi <- as.numeric(gene[,FC])
  sei <- as.numeric(gene[,SE])
  # meta analysis with random models "REML"
  res <- metafor::rma(yi, sei=sei)
  meta <- rbind(meta,c(gs[i],res$QEp,res$pval))
}
...
genesqval <-  genesqval[order(genesqval$QEp),]
...
color <- colorRampPalette(brewer.pal(n = 9, name = "RdBu"))(15) #"BrBG"
topgenes <- full_tables[full_tables$Gene.Symbol %in% genesqval$genes,FC]
