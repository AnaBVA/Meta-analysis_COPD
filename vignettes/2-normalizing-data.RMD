...
qrsh 
cd /mnt/Genoma/amedina/amedinalab/DataDNA/R-projects/Meta-analysis_COPD
 module load r/4.0.1 

## OR
ssh ana@10.200.0.42
cd /home/ana/R-projects/Meta-analysis_COPD/output_data/.out

nohup R -e "rmarkdown::render(here::here('vignettes/2-normalizing-data.RMD'))" > output_data/.out/2-normalizing-data.out &
...
For memmory reasons, we run the normalization using an Rscript 

```{bash,eval = F}
## For running R script
nohup R CMD BATCH R/2-normalizing-data.R &
```


### Libraries

```{r libraries, message=FALSE, warning=FALSE,eval = F}
library(limma)
library(oligo)
library(tidyverse)
```
